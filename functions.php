...
function getItemImage($id, $count = 1, $attributes = null)
{
	$tooltip = '';

	$name = getItemNameById($id);
    $description = Items::getTooltip($id, $count, $attributes);
	if(!empty($description)) {
		$tooltip = ' class="item_image" title="' . $description . '"';
	}
...
function encrypt($str)
{
	...
	// For otserv salt (usually empty in modern TFS)
	if(isset($config['database_salt'])) 
		$str .= $config['database_salt'];

	$encryptionType = isset($config['database_encryption']) ? strtolower($config['database_encryption']) : 'plain';
	
	if($encryptionType !== 'plain')
	{
		// Native TFS $SHA256$<salt>$<hash> format
		if($encryptionType === 'sha256') {
			$saltBytes = random_bytes(16);
			$saltHex = bin2hex($saltBytes);
			$hash = hash('sha256', $saltHex . $str);
...
/**
 * Verify if password matches the stored hash, supporting both the new format
 * ($SHA256$<salt>$<hash>) and the legacy formats.
 */
function verify_password($password, $stored_hash, $legacy_salt = '')
{
	global $config;

	// Check if the stored hash uses the new SHA256 format
	if (strpos($stored_hash, '$SHA256$') === 0) {
		$parts = explode('$', substr($stored_hash, 8)); // remove $SHA256$ prefix
		if (count($parts) === 2) {
			$salt = $parts[0];
			$expected_hash = $parts[1];
			// The C++ server computes SHA256(salt + password)
			$computed_hash = hash('sha256', $salt . $password);
			return hash_equals($expected_hash, $computed_hash);
		}
	}

	// Fallback to legacy encryption
	$legacy_password = $password;
	if (!empty($legacy_salt)) {
		$legacy_password = $legacy_salt . $password;
	}

	return hash_equals($stored_hash, encrypt($legacy_password));
}
...
function getTopPlayers($limit = 5, $skill = 'level') {
	...
	return Cache::remember("top_{$limit}_{$skill}", 2 * 60, function () use ($db, $limit, $skill) {
		...
		if (!in_array($skill, $columns) && $db->hasColumn('players', $skill)) {
			$columns[] = $skill;
		}
