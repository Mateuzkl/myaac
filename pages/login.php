...
if(!empty($login_account) && !empty($login_password))
{
	...
	if($account_logged->isLoaded() && verify_password($login_password, $account_logged->getPassword(), USE_ACCOUNT_SALT ? $account_logged->getCustomField('salt') : '') && (!$limiter->enabled || !$limiter->exceeded($ip))
	)
	{
		if (setting('core.account_mail_verify') && (int)$account_logged->getCustomField('email_verified') !== 1) {
			...
			setSession('password', $account_logged->getPassword());
			if($remember_me) {
